...
- P0 security: Inject CSRF token source in `templates/base.html` and enforce token/header propagation in `static/js/crud-utils.js`.
- P1 regression: Return deterministic JSON CSRF/auth error contracts for AJAX in `app.py` to avoid frontend parser crashes.
- P1 regression: Refactor modal/action event binding in `static/js/main.js` and related view payload wiring in `views/properties.py` for dynamic DOM stability.
