...
class VectorInitializer:
    ...
    def initialize_vector_database(self) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully initialized vector database with {len(properties)} properties")
                return True
            else:
    ...
    def test_vector_search(self) -> bool:
        ...
        try:
            ...
            if recommendations:
                self.logger.info(f"Vector search test successful: {len(recommendations)} recommendations returned")
                
                # Log sample results for debugging
                for i, rec in enumerate(recommendations[:3]):
                    self.logger.info(f"  {i+1}. Property {rec['property'].id}: {rec['hybrid_score']:.1f} score")
