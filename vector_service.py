...
class VectorService:
    ...
    def __init__(self, persist_directory: str = "./chroma_db"):
        ...
        self.properties_collection = self.chroma_client.get_or_create_collection(
            name="properties", metadata={"description": "Property embeddings for semantic search"}
        )

        self.customers_collection = self.chroma_client.get_or_create_collection(
            name="customers", metadata={"description": "Customer preference embeddings"}
        )
    ...
    def _create_customer_text(self, customer: Customer) -> str:
        ...
        """
        budget_range = self._get_price_range_description(
    ...
    def index_properties(self, properties: List[Property]) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully indexed {len(properties)} properties with {len(embeddings[0]) if embeddings else 0}-dimensional embeddings"
            )
            return True
